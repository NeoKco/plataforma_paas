...
- fecha: 2026-04-22
- foco de iteración: tras probar el slice manual de `business-core > Clients`, se corrigió una desviación de comportamiento: ya no corresponde unificar fichas ni mover datos. El flujo quedó reducido a normalizar solo `Organización / Razón social`, se restauró la ficha productiva claramente afectada donde el nombre visible del cliente había quedado pisado por el nombre común de la organización y, en una segunda corrección, la vista dedicada `Nombre común` dejó de depender de vacíos para pasar a detectar candidatos reales por similitud de organización. Además, ya quedó corregido el bloqueo backend que impedía homologar grupos como `Cerrillos/cerrillos` por duplicados históricos del `name` interno aunque el operador solo estuviera cambiando el nombre común visible. Con ese hotfix ya promovido, `maintenance` corrigió una regresión visual y volvió a separar bien los campos: `Cliente` usa el nombre individual/base y la organización común queda solo donde corresponde. En esta última pasada también quedó cerrada la segunda ola visible de `organization addresses` y lectura operacional por organización dentro de `business-core`: `Organizations` ya resume dirección propia, contacto principal y clientes ligados, mientras `Clients` ya cuantifica homologación por organización común y expone ese estado por fila. La documentación activa también quedó armonizada para reflejar solo el comportamiento vigente real: `Duplicados` mantiene su naming actual, `Nombre común` queda documentado como homologación segura de `legal_name`, y las guías ya distinguen explícitamente `organization.name` vs `organization.legal_name`.
- estado general: `Agenda` ya vive como entrada propia en la barra lateral tenant, `platform_admin > Tenants` ya muestra `Postura operativa tenant`, `Contexto de alertas activas` y una franja de prioridad/acción más clara, `Provisioning` ya suma `Plan operativo sugerido` con saltos rápidos a jobs, alertas, observabilidad o DLQ, `Dashboard` ya refleja la misma jerarquía operacional con `Prioridades visibles`, `Acciones rápidas`, labels compactos y una franja `Ruta rápida`, y `Billing` ya entra con la misma lógica visual compactando `Señales abiertas`, `Workspace tenant`, labels cortos y franja `Ruta rápida`; ese frente además cerró la decisión estructural de extraer [OperationalSummaryStrip.tsx](/home/felipe/platform_paas/frontend/src/components/common/OperationalSummaryStrip.tsx) como componente compartido reutilizado por los cuatro workspaces y ya publicado en `staging` y `production`. Dentro de `maintenance`, [MaintenanceWorkOrderDetailModal.tsx](/home/felipe/platform_paas/frontend/src/apps/tenant_portal/modules/maintenance/components/common/MaintenanceWorkOrderDetailModal.tsx) ya muestra `Contacto principal` en la ficha operativa e histórica, [MaintenanceWorkOrdersPage.tsx](/home/felipe/platform_paas/frontend/src/apps/tenant_portal/modules/maintenance/pages/MaintenanceWorkOrdersPage.tsx) y [MaintenanceHistoryPage.tsx](/home/felipe/platform_paas/frontend/src/apps/tenant_portal/modules/maintenance/pages/MaintenanceHistoryPage.tsx) dejan ese dato visible también en la lectura exterior, [MaintenanceReportsPage.tsx](/home/felipe/platform_paas/frontend/src/apps/tenant_portal/modules/maintenance/pages/MaintenanceReportsPage.tsx) ya expone el listado histórico realizado por organización con `cliente`, `contacto principal`, `dirección`, `instalación` y `fecha realizada`, y la corrección final ya separa bien los campos: `Cliente` vuelve a usar el nombre individual/base (`name`) y la organización común homologada (`legal_name`) queda solo en filtros o columnas expresamente rotuladas como organización. Dentro de `business-core`, `Maintenance -> Instalaciones` ya abre `Activos sitio` con foco contextual del mismo sitio, [BusinessCoreAssetsPage.tsx](/home/felipe/platform_paas/frontend/src/apps/tenant_portal/modules/business_core/pages/BusinessCoreAssetsPage.tsx) ya expone búsqueda contextual, filtro por estado, métricas visibles y retorno rápido a `Instalaciones`, el [MaintenanceInstallationTechnicalRecordModal.tsx](/home/felipe/platform_paas/frontend/src/apps/tenant_portal/modules/maintenance/components/common/MaintenanceInstallationTechnicalRecordModal.tsx) ya muestra resumen operativo del inventario reusable del sitio con CTA al inventario completo, la [BusinessCoreClientDetailPage.tsx](/home/felipe/platform_paas/frontend/src/apps/tenant_portal/modules/business_core/pages/BusinessCoreClientDetailPage.tsx) resume activos por dirección/sitio dentro de la ficha del cliente, [BusinessCoreOverviewPage.tsx](/home/felipe/platform_paas/frontend/src/apps/tenant_portal/modules/business_core/pages/BusinessCoreOverviewPage.tsx) ya muestra señal rápida de inventario reusable desde `Resumen`, [BusinessCoreClientsPage.tsx](/home/felipe/platform_paas/frontend/src/apps/tenant_portal/modules/business_core/pages/BusinessCoreClientsPage.tsx) vuelve a quedar como lectura limpia de cartera con columna `Activos`, CTA a la nueva vista de trabajo y lectura operacional del grupo social común, [BusinessCoreOrganizationsPage.tsx](/home/felipe/platform_paas/frontend/src/apps/tenant_portal/modules/business_core/pages/BusinessCoreOrganizationsPage.tsx) ya suma resumen operacional de dirección/contacto/clientes ligados además de la primera ola de `calle + número` y salida a `Google Maps`, y [BusinessCoreCommonOrganizationNamePage.tsx](/home/felipe/platform_paas/frontend/src/apps/tenant_portal/modules/business_core/pages/BusinessCoreCommonOrganizationNamePage.tsx) ya concentra la homologación manual de `Organización / Razón social` mostrando candidatos por similitud real de organización en vez de solo vacíos. La vista nueva agrupa por señales como `RUT / Tax ID`, nombre exacto, nombre muy parecido por prefijo o primeros términos iguales; cuando todo el grupo comparte el mismo `Organización / Razón social`, deja de aparecer. En `production`, además, la ficha `client_id=192 / organization_id=216` volvió a quedar como `Cecilia Tabales` con `legal_name=Los Arbolitos` y contacto principal intacto. [BusinessCoreDuplicatesPage.tsx](/home/felipe/platform_paas/frontend/src/apps/tenant_portal/modules/business_core/pages/BusinessCoreDuplicatesPage.tsx) ya profundiza `installations` hasta fechas técnicas y garantía. El resto del frente sigue igual de alineado: `Duplicados` persiste auditoría visible también para `clients`, `contacts`, `sites` e `installations`, y el importador legacy ya quedó validado en runtime real sobre `empresa-bootstrap`. El hardening de auditoría/reparación tenant-local ya quedó promovido con evidencia real en ambos ambientes, ambos siguen 4/4 limpios en convergencia crítica y `empresa-bootstrap` ya no queda como deuda abierta sino como `accepted_legacy_finance_base_currency:USD`; en repo, además, ya quedaron alineados los punteros de estructura y continuidad (`project-structure.md`, `docs/index.md`, `README.md`, `implementation-governance.md`), el gate post-deploy ya guarda un snapshot JSON del estado de convergencia por ambiente, el script canónico de drift ya expone `secret_posture` y protege por defecto el `.env` legacy ante sincronizaciones accidentales, el wrapper backend ya promueve automáticamente `backend/` desde `/home/felipe/platform_paas` hacia `/opt/platform_paas_staging/backend` o `/opt/platform_paas/backend` antes de tests/restart/gate, el rollback ya no asume que `/opt/...` es git sino que opera desde `SOURCE_REPO_ROOT`, y los wrappers `staging`/`production` ya activan por defecto `RUN_REMOTE_BACKEND_SMOKE_POST_DEPLOY=true` con `REMOTE_BACKEND_SMOKE_TARGET=base`
