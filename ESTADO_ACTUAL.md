...
- fecha: 2026-04-21
- foco de iteración: el hardening transversal de convergencia post-deploy ya dejó cerrado el frente residual de `finance`; tras cerrar también el primer frente de `frontend fino`, el foco real ya pasó a `business-core` de adopción profunda y ya avanzó doce cortes concretos: adopción de `assets` por `maintenance`, trazabilidad visible de `Duplicados`, endurecimiento del importador legacy para sanear texto visible antes de persistirlo, una primera capa de merge guiado/documental visible para `contacts`, una mejora real de legibilidad del `Historial reciente de consolidaciones` usando `diff_rows` y `selections`, una primera capa guiada/documental equivalente para `clients`, otra para `sites`, otra para `installations`, un ajuste visible de `organization addresses` para dejar `Organizations` con captura estructurada y salida operativa a mapa, una primera adopción visible de `assets` dentro de la ficha del cliente, una señal rápida de inventario reusable ya visible desde `Resumen` y ahora una señal rápida de inventario también en la lectura principal de cartera cliente
- estado general: `Agenda` ya vive como entrada propia en la barra lateral tenant, `platform_admin > Tenants` ya muestra `Postura operativa tenant`, `Contexto de alertas activas` y una franja de prioridad/acción más clara, `Provisioning` ya suma `Plan operativo sugerido` con saltos rápidos a jobs, alertas, observabilidad o DLQ, `Dashboard` ya refleja la misma jerarquía operacional con `Prioridades visibles`, `Acciones rápidas`, labels compactos y una franja `Ruta rápida`, y `Billing` ya entra con la misma lógica visual compactando `Señales abiertas`, `Workspace tenant`, labels cortos y franja `Ruta rápida`; ese frente además cerró la decisión estructural de extraer [OperationalSummaryStrip.tsx](/home/felipe/platform_paas/frontend/src/components/common/OperationalSummaryStrip.tsx) como componente compartido reutilizado por los cuatro workspaces y ya publicado en `staging` y `production`. Dentro de `business-core`, `Maintenance -> Instalaciones` ya abre `Activos sitio` con foco contextual del mismo sitio, [BusinessCoreAssetsPage.tsx](/home/felipe/platform_paas/frontend/src/apps/tenant_portal/modules/business_core/pages/BusinessCoreAssetsPage.tsx) ya expone búsqueda contextual, filtro por estado, métricas visibles y retorno rápido a `Instalaciones`, el [MaintenanceInstallationTechnicalRecordModal.tsx](/home/felipe/platform_paas/frontend/src/apps/tenant_portal/modules/maintenance/components/common/MaintenanceInstallationTechnicalRecordModal.tsx) ya muestra resumen operativo del inventario reusable del sitio con CTA al inventario completo, la [BusinessCoreClientDetailPage.tsx](/home/felipe/platform_paas/frontend/src/apps/tenant_portal/modules/business_core/pages/BusinessCoreClientDetailPage.tsx) resume activos por dirección/sitio dentro de la ficha del cliente, [BusinessCoreOverviewPage.tsx](/home/felipe/platform_paas/frontend/src/apps/tenant_portal/modules/business_core/pages/BusinessCoreOverviewPage.tsx) ya muestra señal rápida de inventario reusable desde `Resumen`, y ahora [BusinessCoreClientsPage.tsx](/home/felipe/platform_paas/frontend/src/apps/tenant_portal/modules/business_core/pages/BusinessCoreClientsPage.tsx) también incorpora una columna `Activos` para la lectura principal de cartera, con conteo visible, activos/inactivos, sitios con activos y CTA contextual al sitio con inventario. Además, [BusinessCoreDuplicatesPage.tsx](/home/felipe/platform_paas/frontend/src/apps/tenant_portal/modules/business_core/pages/BusinessCoreDuplicatesPage.tsx) ya no solo detecta y consolida duplicados: ahora muestra `Historial reciente de consolidaciones`, persiste auditoría visible también para `clients`, `contacts`, `sites` e `installations`, ofrece `Ajuste manual previo` + `Diff final por campo` en `contacts`, resume cambios documentales relevantes y origen `auto/manual` cuando el payload trae `diff_rows` y `selections`, extiende ese mismo patrón a `clients` para `estado servicio` y `notas comerciales`, a `sites` para dirección visible, ubicación y notas de referencia, y ahora también a `installations` para identidad técnica visible (`nombre`, `serie`, `fabricante`, `modelo`) y notas/ubicación visibles; estos subcortes ya quedaron publicados también en `staging` y `production`. La primera ola visible de `organization addresses` también quedó más cerrada: [BusinessCoreOrganizationsPage.tsx](/home/felipe/platform_paas/frontend/src/apps/tenant_portal/modules/business_core/pages/BusinessCoreOrganizationsPage.tsx) ya captura `calle` + `número`, deriva `address_line` al guardar y agrega salida directa a `Google Maps` cuando la contraparte ya tiene dirección cargada. El subcorte del importador legacy ya quedó validado en runtime real sobre `empresa-bootstrap` en ambos carriles: [import_ieris_business_core_maintenance.py](/home/felipe/platform_paas/backend/app/scripts/import_ieris_business_core_maintenance.py) ahora sanea marcadores `legacy_*` y placeholders en notas/descripciones visibles de `business-core` y `maintenance`, actualiza también filas legacy existentes de `maintenance` y usa una verificación post-import alineada con `empresa + clientes` para `organizations`; en `staging` además completó `10` `work_orders/status_logs/visits` históricos que faltaban. El hardening de auditoría/reparación tenant-local ya quedó promovido con evidencia real en ambos ambientes, ambos siguen 4/4 limpios en convergencia crítica y `empresa-bootstrap` ya no queda como deuda abierta sino como `accepted_legacy_finance_base_currency:USD`; en repo, además, ya quedaron alineados los punteros de estructura y continuidad (`project-structure.md`, `docs/index.md`, `README.md`, `implementation-governance.md`), el gate post-deploy ya guarda un snapshot JSON del estado de convergencia por ambiente, el script canónico de drift ya expone `secret_posture` y protege por defecto el `.env` legacy ante sincronizaciones accidentales, el wrapper backend ya promueve automáticamente `backend/` desde `/home/felipe/platform_paas` hacia `/opt/platform_paas_staging/backend` o `/opt/platform_paas/backend` antes de tests/restart/gate, el rollback ya no asume que `/opt/...` es git sino que opera desde `SOURCE_REPO_ROOT`, y los wrappers `staging`/`production` ya activan por defecto `RUN_REMOTE_BACKEND_SMOKE_POST_DEPLOY=true` con `REMOTE_BACKEND_SMOKE_TARGET=base`
