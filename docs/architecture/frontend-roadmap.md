...
- `business-core` y `maintenance` ya absorbieron también el helper transversal `pickLocalizedText()` en navegación, placeholders, catálogos, paneles/modales compartidos y páginas densas como `Clientes`, `Taxonomías`, `Ficha de cliente`, `Grupos`, `Miembros del grupo`, `Instalaciones`, `Mantenciones abiertas`, `Costos de mantenciones` e `Historial técnico`, y sus `Overview` ya exponen `AppSpotlight` para alinear las nuevas pantallas con la convención visual del módulo piloto
- la pasada transversal restante sobre `business-core` y `maintenance` baja ahora a residual editorial no bloqueante: quedan páginas densas como `Duplicados`, `Pendientes`, partes de `Resumen técnico` y algunos catálogos todavía con ternarios/manual copy, pero ese remanente queda movido al backlog para no retrasar la salida a terreno del PaaS
...
- la internacionalizacion transversal del frontend ya quedó resuelta en la superficie visible principal, helpers compartidos, formularios densos y estados operativos; `business-core` y `maintenance` ya quedaron alineados también en sus capas compartidas nuevas, por lo que lo que resta baja a editorial secundaria puntual, ayudas largas y futuras pantallas nuevas que se abran fuera del alcance actual
...
- `platform_admin` ya adopta también el patrón de alta bajo demanda en formularios sensibles: `Tenants` y `Usuarios de plataforma` abren modales de creación y dejan catálogo/lectura como primer plano
- ese patron deja de ser puntual y pasa a convención: cualquier CRUD nuevo del proyecto debería priorizar lectura y abrir la creación solo bajo demanda
...
- no volver a frenar releases operativos por remanentes editoriales de i18n/copy mientras el flujo visible principal ya esté validado; esos residuos deben quedar documentados como backlog y no mezclarse con la salida a terreno
...
- el bloque base de usuarios tenant ya cubre lectura clara de `core.users`, `core.users.active`, `core.users.monthly` y limites por rol sin dejar mensajes crudos de error, incluyendo ya los casos visibles de creación bloqueada por `core.users.monthly` y de creación/reactivación bloqueada por `core.users.admin`
