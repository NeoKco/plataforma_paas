...
- `business-core` y `maintenance` ya absorbieron también el helper transversal `pickLocalizedText()` en navegación, placeholders, catálogos, paneles/modales compartidos y páginas densas como `Taxonomías`, `Ficha de cliente` e `Historial técnico`, y sus `Overview` ya exponen `AppSpotlight` para alinear las nuevas pantallas con la convención visual del módulo piloto
