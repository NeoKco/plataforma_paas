...
- `business-core` y `maintenance` ya absorbieron también el helper transversal `pickLocalizedText()` en navegación, placeholders, catálogos y modales/paneles compartidos, y sus `Overview` ya exponen `AppSpotlight` para alinear las nuevas pantallas con la convención visual del módulo piloto
