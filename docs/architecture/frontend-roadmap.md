...
- ya existe un stack E2E browser local con `Playwright` en `frontend/`, con smoke inicial sobre `platform_admin`, `tenant_portal` y creación básica de transacción en `finance`; falta ampliarlo a provisioning, lifecycle tenant y más regresión funcional
