...
- ya existe un stack E2E browser local con `Playwright` sobre `frontend/`, cubriendo login `platform_admin`, login `tenant_portal` y `finance` en creación, adjunto, anulación y conciliación; queda pendiente ampliarlo a lifecycle tenant, provisioning, archive/restore y enforcement visible de límites
