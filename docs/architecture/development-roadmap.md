...
- ya existe un stack E2E browser local con `Playwright` sobre `frontend/`, cubriendo login `platform_admin`, lifecycle tenant base (`create/archive/restore`), login `tenant_portal` y `finance` en creación, adjunto, anulación y conciliación; queda pendiente ampliarlo a provisioning más profundo y enforcement visible de límites
