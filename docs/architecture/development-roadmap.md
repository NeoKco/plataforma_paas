...
- ya existe un stack E2E browser local con `Playwright` sobre `frontend/`, cubriendo login `platform_admin`, lifecycle tenant base (`create/archive/restore`), acceso rápido desde `Tenants` hacia `tenant_portal`, visibilidad de jobs nuevos en `Provisioning`, ejecución manual de jobs `pending`, requeue de jobs `failed`, disparo de `schema auto-sync`, y además un smoke broker-only para requeue batch de filas DLQ filtradas; junto a eso cubre login `tenant_portal` y `finance` en creación, adjunto, anulación y conciliación; queda pendiente ampliarlo a variantes más finas de DLQ y enforcement visible de límites
