...
- ya existe un primer stack E2E browser local con `Playwright` sobre `frontend/`, cubriendo login `platform_admin`, login `tenant_portal` y smoke base de `finance`; queda pendiente ampliarlo a lifecycle tenant, provisioning, archive/restore y enforcement visible de límites
