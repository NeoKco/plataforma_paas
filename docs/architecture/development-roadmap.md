...
- ya existe un stack E2E browser local con `Playwright` sobre `frontend/`, cubriendo login `platform_admin`, lifecycle tenant base (`create/archive/restore`), acceso rápido desde `Tenants` hacia `tenant_portal`, visibilidad de jobs nuevos en `Provisioning`, ejecución manual de jobs `pending`, requeue de jobs `failed`, disparo de `schema auto-sync` y variantes broker-only de DLQ; junto a eso cubre `tenant_portal` para enforcement visible de límites de usuarios activos, de `finance`, de la precedencia entre `finance.entries` y `finance.entries.monthly`, del cupo mensual `finance.entries.monthly`, de los cupos mensuales por tipo `finance.entries.monthly.income` / `finance.entries.monthly.expense`, y del mantenimiento básico de cuentas/categorías, además de `finance` en creación, adjunto, anulación y conciliación; queda pendiente ampliarlo a más regresión funcional
