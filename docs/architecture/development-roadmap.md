...
- ya existe un stack E2E browser local con `Playwright` sobre `frontend/`, cubriendo login `platform_admin`, lifecycle tenant base (`create/archive/restore`), visibilidad de jobs nuevos en `Provisioning`, ejecución manual de jobs `pending`, requeue de jobs `failed`, disparo de `schema auto-sync`, login `tenant_portal` y `finance` en creación, adjunto, anulación y conciliación; queda pendiente ampliarlo a DLQ de provisioning y enforcement visible de límites
