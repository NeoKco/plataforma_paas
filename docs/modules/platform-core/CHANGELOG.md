...
- este corte queda validado en `repo` con `npm run build` y `npx playwright test --list`, y publicado en `staging`; el smoke nuevo todavía necesita estabilización final antes de promover el cambio a `production`
