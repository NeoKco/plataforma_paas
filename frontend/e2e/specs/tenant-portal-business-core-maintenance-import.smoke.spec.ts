...
test("tenant portal shows imported business core and maintenance data from ieris_app", async ({
  ...
  await expect(
    page.getByText(/Ajuste manual previo|Manual pre-merge adjustment|No se encontraron organizaciones duplicadas|No duplicate organizations were found/i).first()
  ).toBeVisible();
  await expect(
    page.getByText(/Diff final por campo|Final field diff|No se encontraron organizaciones duplicadas|No duplicate organizations were found/i).first()
  ).toBeVisible();
  await expect(
  ...
  if ((await openWorkOrderRows.count()) > 0) {
    ...
    await expect(getFieldControl(visitsDialog, /Tipo de visita|Visit type/i)).toBeVisible();
    await expect(getFieldControl(visitsDialog, /Resultado operativo|Operational result/i)).toBeVisible();
    ...
    const visitSyncWindowText = rescheduleDialog.getByText(/Ventana a sincronizar|Window to sync/i);
    if ((await visitSyncWindowText.count()) > 0) {
      await expect(visitSyncWindowText).toBeVisible();
    }
