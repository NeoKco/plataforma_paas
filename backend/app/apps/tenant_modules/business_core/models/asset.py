...
class BusinessAsset(TenantBase):
    ...
    asset_type_id: Mapped[int] = mapped_column(
        Integer,
        ForeignKey("business_asset_types.id", ondelete="RESTRICT"),
        nullable=False,
        index=True,
    )
    name: Mapped[str] = mapped_column(String(150), nullable=False, index=True)
    asset_code: Mapped[str | None] = mapped_column(String(60), nullable=True, unique=True, index=True)
    serial_number: Mapped[str | None] = mapped_column(String(120), nullable=True, index=True)
    manufacturer: Mapped[str | None] = mapped_column(String(120), nullable=True)
    model: Mapped[str | None] = mapped_column(String(120), nullable=True)
    asset_status: Mapped[str] = mapped_column(String(40), nullable=False, default="active", index=True)
